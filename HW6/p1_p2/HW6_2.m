...
[coor, conn, ndime, mate, nnode, nelem, nelnd, npres, pres, nload, load, ntrac, trac] = ReadInput('hw6_2_inputfile.h5');
...
for i = 2:nstpho+1
    ...
    while(erri > tol && niti < maxit)
        ...
        [fglob, sigm_glob] = GlobResi(ndime,nnode,nelem,nelnd,mate,coor,conn,wglob);
        ...
        for j = 1:npres
            ...
            kglob(ir,ir) = 1;
            bglob(ir,1) = -wglob(ir) + faci*pres(j,3);
        end
        dwglob = kglob\bglob;
    ...
    disp(num2str(wglob(12,1)));
    disp(num2str(fglob(12,1)));


    e_plot = [e_plot, abs(wglob(12,1)-wglob(10,1))*50/14];
    s_plot = [s_plot, 5*abs(fglob(12,1))];
end
figure(5);
plot(e_plot, s_plot);
xlabel('Strain');
